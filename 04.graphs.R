...
plot(as.Date(data$date),exp(timeeff),type="l",xlab="Time",ylab="Daily deaths",
  ylim=c(100,160),lwd=1.5,frame.plot=T,col=2)
...
arg3D <- list(y=0:20,ticktype="detailed",theta=230,ltheta=150,phi=40,lphi=30,
  ...
  border=grey(0.2),col="lightskyblue")

pdf("figure2.pdf",height=8,width=7)
...
pdf("figure3.pdf",height=3.5,width=9)
...
for(j in 1:2) {
  ...
  lines(0:20,exp(pred[[j]][[1]][as.character(10),]),lty=5,lwd=1.5,col=2)
  mtext("Lag-response at 10 PPM",cex=0.8)
  legend("top",c("True","DLM (average)","DLM (sample)"),lty=c(1,5,1),
    lwd=1,col=c(1,2,grey(0.85)),cex=0.7,bty="n",inset=0.05)
}

dev.off()


pdf("figure4.pdf",height=5,width=8)
...
for(j in 3:4) {
  ...
  lines(0:20,exp(pred[[j]][[1]][as.character(30),]),lty=5,lwd=1.5,col=2)
  ...
  lines(0:20,exp(pred[[j]][[1]][as.character(-15),]),lty=5,lwd=1.5,col=2)
...
pdf("figure5.pdf",height=6.5,width=7)
...
for(j in seq(true)) {
  ...
  lines(x,exp(cumpred[[j]][[1]]),lty=5,lwd=1.5,col=2)
  lines(x,exp(cumpred[[j]][[2]]),lty=2,lwd=1.5,col=3)
  lines(x,exp(cumpred[[j]][[4]]),lty=4,lwd=1.5,col=4)
  title(names(combsim)[j],cex=0.8)
  legend("top",c("True",mod[c(1,2,4)]),lty=c(1,5,2,4),lwd=1,col=1:4,cex=0.7,
